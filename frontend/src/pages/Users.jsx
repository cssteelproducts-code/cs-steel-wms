...
export default function Users() {
  ...
  const handleSaveUser = async () => {
    ...
    try {
      ...
      if (modal === 'create') {
        ...
        res = await api.put(`/users/${selectedUser.UserID}`, form);
      }
      if (res.data.success) {
  ...
  const handleSavePermissions = async () => {
    setSaving(true);
    try {
      const permsArr = MENUS.map(m => ({
        menuCode: m.code, menuName: m.name, ...permissions[m.code]
      }));
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex gap-2">
        <button onClick={() => setTab('users')}
          className={`px-4 py-2 rounded-lg text-sm font-medium ${tab === 'users' ? 'bg-blue-600 text-white' : 'bg-steel-700 text-steel-300 hover:text-white'}`}>
          <UsersIcon size={14} className="inline mr-1" />ผู้ใช้งาน ({users.length})
        </button>
        <button onClick={() => setTab('roles')}
          className={`px-4 py-2 rounded-lg text-sm font-medium ${tab === 'roles' ? 'bg-blue-600 text-white' : 'bg-steel-700 text-steel-300 hover:text-white'}`}>
        ...
        <div className="card">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {users.map(u => (
                  <tr key={u.UserID} className="border-b border-steel-700/50 hover:bg-steel-700/30">
                    <td className="table-cell font-mono text-blue-400">{u.Username}</td>
                    <td className="table-cell text-white">{u.FullName}</td>
                    <td className="table-cell hide-mobile">
                      <span className="px-2 py-0.5 bg-steel-600 rounded-full text-xs">{u.RoleName}</span>
                    </td>
                    <td className="table-cell hide-mobile">{u.WarehouseName || '-'}</td>
                    <td className="table-cell text-center">
                      <span className={`px-2 py-0.5 rounded-full text-xs ${u.IsActive ? 'bg-emerald-900/30 text-emerald-400' : 'bg-red-900/30 text-red-400'}`}>
                        {u.IsActive ? 'ใช้งาน' : 'ระงับ'}
                      </span>
                    </td>
                    <td className="table-cell hide-mobile text-steel-500 text-xs">{formatDateTime(u.LastLogin) || 'ยังไม่เคย'}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEdit(u)} className="p-1.5 rounded hover:bg-steel-600 text-steel-400 hover:text-white transition-colors">
                          <Edit size={14} />
                        </button>
                        {u.IsActive ? (
                          <button onClick={() => handleDeactivate(u.UserID)} className="p-1.5 rounded hover:bg-red-900/30 text-steel-400 hover:text-red-400 transition-colors">
                            <UserX size={14} />
                          </button>
                        ) : null}
                      </div>
      ...
      {tab === 'roles' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {roles.map(role => (
            <div key={role.RoleID} className="card hover:border-blue-500/30 transition-all">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h4 className="text-white font-semibold">{role.RoleName}</h4>
                  <p className="text-steel-400 text-xs mt-0.5">{role.Description}</p>
                </div>
            ...
            <div className="space-y-3">
              ...
              {modal === 'edit' && (
                <div className="flex items-center gap-2">
                  ...
                  <label htmlFor="isActive" className="text-steel-300 text-sm cursor-pointer">เปิดใช้งาน</label>
                </div>
              )}
            </div>
      ...
      {modal === 'permissions' && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-steel-800 border border-steel-700 rounded-2xl w-full max-w-2xl p-6 my-4">
            <h3 className="text-lg font-bold text-white mb-1">กำหนดสิทธิ์: {selectedRole?.RoleName}</h3>
            <p className="text-steel-400 text-sm mb-4">{selectedRole?.Description}</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {MENUS.map(menu => (
                    <tr key={menu.code} className="border-b border-steel-700/50">
                      <td className="px-3 py-2 text-sm text-steel-200">{menu.name}</td>
